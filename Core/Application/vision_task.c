...
uint8_t usart1_rx_buf[2][USART_RX_BUF_LENGHT];
uint8_t usart1_vision_tx_buf[2][USART_TX_BUF_LENGHT];
...
//    init_referee_struct_data();
    ...
    usart1_rx_init(usart1_rx_buf[0], usart1_rx_buf[1], USART_RX_BUF_LENGHT);
...
//    init_referee_struct_data();
    ...
    usart1_tx_init(usart1_vision_tx_buf[0], usart1_vision_tx_buf[1], USART_TX_BUF_LENGHT);
    TickType_t LoopStartTime;
    while (1) {
        LoopStartTime = xTaskGetTickCount();
//        referee_unpack_fifo_data();
...
//                    SEGGER_RTT_WriteString(0, "ST0DMA_1");
                            ...
                            memset(&usart1_vision_tx_buf[1], 0, USART_TX_BUF_LENGHT);
                            ...
                            if (fifo_s_used(&vision_tx_fifo)) {
                                if (fifo_s_used(&vision_tx_len_fifo)) {
                                    vision_dma_send_data_len = fifo_s_get(&vision_tx_len_fifo);
                                    memset(&usart1_vision_tx_buf[0], 0, USART_TX_BUF_LENGHT);
...
//                    SEGGER_RTT_WriteString(0, "ST0DMA_0");
            ...
            SEGGER_RTT_printf(0, "handle1=%d\r\n", Matlab_No_DMA_IRQHandler);
            if (huart1.hdmatx->Instance->CR !=)) {
